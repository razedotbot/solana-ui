...
import { CheckCircle, ChevronLeft, ChevronRight, Info, Search, ArrowDown } from 'lucide-react';
...
const STEPS_BURN = ['Token Address', 'Select Wallet', 'Burn Details', 'Review'];
...
export const BurnPage: React.FC = () => {
  ...
  const { solBalances, showToast } = useAppContext();
  
  const [currentStep, setCurrentStep] = useState(0);
  const [sourceWallet, setSourceWallet] = useState<string>('');
  const [tokenAddress, setTokenAddress] = useState<string>(tokenAddressParam);
  ...
  const handleBurn = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const prepareResponse = await fetch(`https://utils.fury.bot/solana/wallets/burn`, {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-app-primary text-app-tertiary">
      ...
      {/* Progress Indicator */}
      <div className="bg-app-secondary border-b border-app-primary-20">
        ...
        <form onSubmit={currentStep === STEPS_BURN.length - 1 ? handleBurn : (e) => e.preventDefault()}>
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              ...
              <div className="mb-3 flex space-x-2">
                <div className="relative flex-grow">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 bg-app-tertiary border border-app-primary-30 rounded-lg text-sm text-app-primary focus:outline-none focus-border-primary transition-all font-mono tracking-wider"
                    placeholder="SEARCH WALLETS_"
                  />
                </div>
                
                <select 
                  className="bg-app-tertiary border border-app-primary-30 rounded-lg px-3 text-sm text-app-primary focus:outline-none focus-border-primary transition-all font-mono"
                  value={sortOption}
                ...
                <button
                  type="button"
                  className="p-2 bg-app-tertiary border border-app-primary-30 rounded-lg text-app-secondary hover:text-app-primary hover-border-primary transition-all"
              ...
              <div className="mb-3">
                <select 
                  className="w-full bg-app-tertiary border border-app-primary-30 rounded-lg p-2 text-sm text-app-primary focus:outline-none focus-border-primary transition-all font-mono"
                  value={balanceFilter}
                    ...
                    filterWallets(wallets, searchTerm).map((wallet) => (
                      ...
                      >
                        ...
                        <div className="flex-1 flex justify-between items-center">
                          <span className="font-mono text-sm text-app-primary">{getWalletDisplayName(wallet)}</span>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className="text-xs px-2 py-0.5 bg-primary-10 hover:bg-primary-20 border border-app-primary-30 color-primary rounded-lg transition-all font-mono"
                          >
                            MAX
                          </button>
                        </div>
                        <input
                          ...
                          className="w-full pl-4 pr-16 py-3 bg-app-primary border border-app-primary-30 rounded-lg text-app-primary focus:outline-none focus-border-primary transition-all font-mono tracking-wider"
                        />
                      </div>

                      {amount && parseFloat(amount) > 0 && (
                        <div className="p-4 border border-app-primary-30 rounded-lg bg-primary-05">
                          <div className="flex justify-between items-center">
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-app-tertiary rounded-lg border border-app-primary-30 p-4">
                  <h4 className="text-base font-semibold text-app-primary font-mono mb-6">
                  ...
                  <div className="flex flex-col items-center justify-center space-y-6">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <div className="text-lg font-semibold color-primary font-mono">
                        {Math.max(0, getSelectedTokenBalance() - parseFloat(amount)).toFixed(4)} {getSelectedTokenSymbol()}
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-primary-05 border border-app-primary-20 rounded-lg p-3">
                <div className="flex items-start color-primary text-sm">
                  ...
                  <span className="font-mono leading-relaxed">
                    <span className="font-bold">WARNING:</span> This burn operation is permanent and irreversible.
                  </span>
                </div>
              </div>
                ...
                >
                  ...
                  <span className="text-sm text-app-primary leading-relaxed font-mono select-none">
                    I confirm that I want to burn <span className="color-primary font-medium">
                      {amount} {getSelectedTokenSymbol()}
                    </span>. I understand this action cannot be undone.
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-between mt-8">
            <button
              ...
              className={`px-4 py-2 bg-app-tertiary border border-app-primary-30 hover-border-primary rounded-lg transition-all flex items-center ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {currentStep === 0 ? (
