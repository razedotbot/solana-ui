        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-6">
          ...
          <div className="flex flex-col xl:flex-row gap-6">
            ...
            <div className="xl:w-56 flex-shrink-0">
              <div className="xl:sticky xl:top-6 flex xl:flex-col gap-1 overflow-x-auto pb-2 xl:pb-0 no-scrollbar">
                {TABS.map((tab) => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`flex-shrink-0 xl:w-full flex items-center gap-2 xl:gap-3 px-3 xl:px-4 py-2 xl:py-3 rounded-xl font-mono text-sm transition-all duration-200 group ${
                      activeTab === tab.id
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <div className={`text-[10px] whitespace-nowrap ${activeTab === tab.id ? "text-black/60" : "text-app-secondary-40"}`}>
                        {tab.description}
                      </div>
                    </div>
                    {activeTab === tab.id && (
                      <ChevronRight size={16} className="ml-auto text-black hidden xl:block" />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="xl:w-96 flex-shrink-0 order-last">
              <DeployPreview
                tokenData={tokenData}
                selectedWallets={selectedWallets}
                walletAmounts={walletAmounts}
