...
import {
  ...
  formatTokenBalance as _formatTokenBalance,
...
export interface BalanceRefreshOptions {
  /** Refresh strategy: 'sequential' (one at a time), 'batch' (in batches), 'parallel' (all at once) */
  strategy?: BalanceRefreshStrategy;
  /** Batch size for batch mode (default: 5) */
  batchSize?: number;
  /** Delay between operations in milliseconds (default: 50) */
  delay?: number;
  /** Only fetch if balance is zero or null */
 ...
 * - sequential: Process one wallet at a time (safest, slowest)
 * - batch: Process wallets in batches (balanced)
 * - parallel: Process all wallets at once (fastest, may hit rate limits)
 */
export const fetchWalletBalances = async (
  ...
  // Parse options - support both legacy boolean and new options object
  ...
  /**
   * Process a single wallet's balance fetch
   */
  const processWallet = async (wallet: WalletType): Promise<void> => {
    try {
      // Create a new connection for each wallet to rotate endpoints (if using RPC manager)
  ...
  /**
   * Update state with current balances (used for progressive updates)
   */
  ...
  switch (strategy) {
    case "sequential": {
      ...
      for (let i = 0; i < wallets.length; i++) {
        ...
        updateState(); // Update after each wallet for progressive feedback

        // Add delay between wallets (except for the last one)
    ...
    case "batch": {
      ...
      for (let i = 0; i < wallets.length; i += batchSize) {
        ...
        updateState(); // Update after each batch for progressive feedback
    ...
    case "parallel": {
      ...
      updateState(); // Update once at the end
      break;
    }

    default: {
      // Fallback to batch strategy
      for (let i = 0; i < wallets.length; i += batchSize) {
        const batch = wallets.slice(i, Math.min(i + batchSize, wallets.length));
        await Promise.all(batch.map((wallet) => processWallet(wallet)));
        updateState();
  ...
  try {
    ...
    if (encryptedData) {
      try {
        ...
        if (oldWallets) {
          console.info(
            "🔄 Loading from old unencrypted storage and migrating...",
          );
...
export const saveQuickBuyPreferencesToCookies = (
  preferences: QuickBuyPreferences,
): void => {
  Cookies.set(QUICK_BUY_COOKIE_KEY, JSON.stringify(preferences), {
    expires: 30,
  });
};

export const loadQuickBuyPreferencesFromCookies =
  (): QuickBuyPreferences | null => {
...
export const saveTradingStrategiesToCookies = (
  strategies: TradingStrategy[],
): void => {
  Cookies.set(TRADING_STRATEGIES_COOKIE_KEY, JSON.stringify(strategies), {
    expires: 30,
  });
};
...
export const saveSplitSizesToCookies = (sizes: number[]): void => {
  try {
    Cookies.set(SPLIT_SIZES_COOKIE_KEY, JSON.stringify(sizes), {
      expires: 365,
    }); // 1 year expiry
  } catch (error) {
