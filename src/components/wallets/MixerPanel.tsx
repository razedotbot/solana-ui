...
export const MixerPanel: React.FC<MixerPanelProps> = ({
  ...
  const handleFund = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const result = await batchMixBaseCurrency(
        senderWallet,
        recipientWallets,
        baseCurrency,
      );
      ...
      if (result.success) {
        showToast(`${baseCurrency.symbol} mixed successfully`, "success");
        resetForm();
        onClose();
      } else {
      ...
      showToast(
        "Mixing failed: " + (errorMessage || "Unknown error"),
        "error",
      );
    } finally {
      setIsSubmitting(false);
  ...
  useModalStyles(isOpen, "mixer-modal-styles");
        ...
        <div className="relative z-10 p-4 flex justify-between items-center border-b border-app-primary-40">
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-semibold text-app-primary font-mono">
              <span className="color-primary">/</span> {baseCurrency.symbol} MIXER{" "}
            ...
            <div className="flex flex-col space-y-4 animate-[fadeIn_0.3s_ease]">
              <div className="w-full space-y-4">
                ...
                <ConfirmCheckbox
                  ...
                  label="I CONFIRM THIS MIXING OPERATION"
                />
              </div>
