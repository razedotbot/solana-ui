...
const UnifiedWalletManager: React.FC<UnifiedWalletManagerProps> = ({
  ...
  const loadLists = type === 'copytrade' ? loadCopyTradeWalletLists : loadWhitelistLists;
  const saveLists = type === 'copytrade' ? saveCopyTradeWalletLists : saveWhitelistLists;
  const createList = type === 'copytrade' ? createWalletList : createWhitelistList;
  ...
  const filteredLists = savedLists.filter(list =>
    ...
    list.addresses.some(addr => addr.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const handleFileImport = (event: React.ChangeEvent<HTMLInputElement>, listId?: string): void => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-mono font-medium text-zinc-200 flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-2.5 py-1.5 bg-zinc-800 border border-zinc-700 rounded-md text-xs font-mono text-zinc-300 
                       hover:bg-zinc-700 hover:text-zinc-100 transition-colors flex items-center gap-1.5"
          ...
          {currentAddresses.length > 0 && (
            <button
              ...
              className="px-2.5 py-1.5 bg-emerald-500/20 border border-emerald-500/30 rounded-md text-xs font-mono text-emerald-400 
      ...
      {savedLists.length > 3 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md font-mono text-sm text-zinc-200 
                       focus:outline-none focus:border-emerald-500/50 transition-colors placeholder:text-zinc-600"
          />
        </div>
      )}
      ...
      {isCreatingList && (
        <div className="flex items-center gap-2 p-3 bg-zinc-800/50 border border-zinc-700 rounded-lg">
          <input
            ...
            className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-700 rounded-md font-mono text-sm text-zinc-200 
                       focus:outline-none focus:border-emerald-500/50 transition-colors placeholder:text-zinc-600"
          ...
          <button
            ...
            className="p-2 bg-emerald-500/20 border border-emerald-500/30 rounded-md text-emerald-400 
                       hover:bg-emerald-500/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Save className="w-4 h-4" />
          </button>
          <button
            onClick={() => { setIsCreatingList(false); setNewListName(''); }}
            className="p-2 bg-zinc-700 rounded-md text-zinc-400 hover:bg-zinc-600 hover:text-zinc-200 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      {savedLists.length === 0 ? (
        <div className="text-center py-8 text-zinc-500 font-mono text-sm">
          ...
          <p className="text-xs mt-1 text-zinc-600">Import a file or save current addresses</p>
        </div>
                ...
                {isEditingList === list.id ? (
                  <div className="flex items-center gap-2 flex-1">
                    <input
                      ...
                      className="flex-1 px-2 py-1 bg-zinc-800 border border-zinc-700 rounded font-mono text-sm text-zinc-200 
                                 focus:outline-none focus:border-emerald-500/50"
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') handleUpdateListName(list.id);
                        if (e.key === 'Escape') { setIsEditingList(null); setEditedListName(''); }
                      }}
                      autoFocus
                    />
                    <button
                    ...
                    <button
                      onClick={() => { setIsEditingList(null); setEditedListName(''); }}
                  ...
                  <>
                    ...
                    >
                      <span className={`
                        ...
                        ${selectedListId === list.id ? 'bg-emerald-400' : 'bg-zinc-600 group-hover:bg-zinc-500'}
                      `} />
                      <div>
                        <span className="font-mono text-sm text-zinc-200">{list.name}</span>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => { setIsEditingList(list.id); setEditedListName(list.name); }}
                        className="p-1.5 rounded text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 transition-colors"
                        title="Edit"
                      >
                      ...
                      <button
                        ...
                        className="p-1.5 rounded text-zinc-500 hover:text-red-400 hover:bg-red-900/20 transition-colors"
                        title="Delete"
                      >
                ...
                <div className="px-3 pb-3 border-t border-zinc-800">
                  ...
                  <div className="flex items-center gap-2 mt-3 mb-2">
                    <input
                      ...
                      className="flex-1 px-2 py-1.5 bg-zinc-800 border border-zinc-700 rounded font-mono text-xs text-zinc-200 
                                 focus:outline-none focus:border-emerald-500/50 placeholder:text-zinc-600"
                      onKeyDown={(e) => e.key === 'Enter' && handleAddAddress(list.id)}
                    />
                    <button
                      onClick={() => handleAddAddress(list.id)}
                      disabled={!newAddress.trim()}
                      className="p-1.5 bg-zinc-800 border border-zinc-700 rounded text-zinc-400 
                  ...
                  {list.addresses.length === 0 ? (
                    <div className="text-center py-3 text-zinc-600 font-mono text-xs">
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {list.addresses.map(address => (
                        <div 
                          key={address}
                          className="flex items-center justify-between p-2 bg-zinc-800/50 rounded group"
                        >
                          <span className="font-mono text-xs text-zinc-300" title={address}>
                            {formatAddress(address)}
                          </span>
                          <button
                            onClick={() => handleRemoveAddress(list.id, address)}
                            className="p-0.5 rounded text-zinc-600 hover:text-red-400 
