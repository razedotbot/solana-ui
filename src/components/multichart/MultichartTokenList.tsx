    ...
    >
      <div className="bg-app-primary border border-app-primary-40 rounded-lg shadow-xl min-w-[300px] max-h-[350px] overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-app-primary-40 bg-app-primary-60">
          <button
            onClick={onSelectAll}
            className="px-2 py-1 text-[10px] font-mono bg-app-quaternary border border-app-primary-40 text-app-secondary rounded hover:bg-app-primary-20 transition-colors"
          >
            All
          </button>
          <button
...
const TokenRow: React.FC<{
  ...
  const handleTrade = useCallback(
    async (isBuy: boolean, amount: string) => {
      ...
      try {
        ...
        if (result.success) {
          showToast(
            `${token.symbol || "Token"}: ${isBuy ? "Buy" : "Sell"} submitted`,
            "success",
          );
        } else {
    ...
    [token, selectedDex, wallets, baseCurrencyBalances, showToast],
  );
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        {isActive && (
          <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-app-primary-color/20 border border-app-primary-color/40">
            ...
            <span className="text-[8px] font-mono color-primary font-semibold">
              VIEWING
            </span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 min-w-0 w-[120px]">
          {token.imageUrl ? (
            <img
              src={token.imageUrl}
              alt=""
              className="w-6 h-6 rounded-full flex-shrink-0 border border-app-primary-40"
              onError={(e) => {
          ...
          <div className="min-w-0 flex-1">
            <div
              className={`text-xs font-mono font-bold truncate ${isActive ? "color-primary" : "text-app-primary"}`}
            >
              {token.symbol || "Unknown"}
            </div>
            <div className="text-[9px] font-mono text-app-secondary-60">
        ...
        <div className="flex items-center gap-4 text-[10px] font-mono flex-1">
          ...
          <div className="w-[60px]">
            ...
            <div className="text-app-primary">
              ${formatMcap(stats?.marketCap)}
            </div>
          </div>
          <div className="w-[70px]">
            <div className="text-app-secondary-60">PnL</div>
            <div
              className={`flex items-center gap-0.5 ${isPnlPositive ? "color-primary" : "text-warning"}`}
            >
              {isPnlPositive ? (
                <TrendingUp size={10} />
              ) : (
...
export const MultichartTokenList: React.FC<MultichartTokenListProps> = ({
  ...
  const { tokens, activeTokenIndex, setActiveToken, removeToken } =
    useMultichart();
  ...
  const handleSelectAll = useCallback((): void => {
    ...
    const updated = wallets.map((w) => ({
      ...w,
      isActive: w.isArchived ? w.isActive : !allActive,
    }));
    setWallets(updated);
    saveWalletsToCookies(updated);
  }, [wallets, setWallets]);
  ...
  const handleSelectAllWithBalance = useCallback((): void => {
    const withBalance = wallets.filter((w) => {
      ...
      return (
        (baseCurrencyBalances.get(w.address) || 0) > 0 ||
        (tokenBalances.get(w.address) || 0) > 0
      );
    });
  ...
  return (
    <div className="h-full bg-app-primary flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-app-primary-40 bg-app-primary-60 flex-shrink-0">
        <span className="text-[10px] font-mono text-app-secondary-60 uppercase">
          Trading Panel
        </span>
        <button
          ref={walletBtnRef}
      ...
      <div className="flex-1 overflow-y-auto">
        {tokens.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-4">
            ...
            <div className="text-sm font-mono text-app-secondary-60 mb-2">
              No tokens added
            </div>
            <button
              onClick={onAddToken}
